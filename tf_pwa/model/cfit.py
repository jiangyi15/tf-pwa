...
class ModelCfitExtended(Model):
    ...
    def nll(
        ...
        n_exp = int_mc / (1 - self.w_bkg)
        nll_0 = (
            -tf.reduce_sum(tf.cast(weight, ln_data.dtype) * ln_data)
            - sw * tf.math.log(n_exp)
            + n_exp
        )
        return nll_0
    ...
    def nll_grad_batch(self, data, mcdata, weight, mc_weight):
        ...
        ret = (
            -ll
            - sw * tf.math.log(int_sig / (1 - self.w_bkg))
            + int_sig / (1 - self.w_bkg)
        )

        g_ll_sig, g_ll_bg = g_ll[-2], g_ll[-1]
        g = [
            -g_ll[i]
            - g_int_sig[i] * g_ll_sig
            - g_int_bg[i] * g_ll_bg
            - sw / int_sig * g_int_sig[i]
            + g_int_sig[i] / (1 - self.w_bkg)
            for i in range(len(var))
        ]
        return ret, g
    ...
    def nll_grad_hessian(
        ...
        h = (
            ...
            * (
                h_int_sig / int_sig
                - np.outer(g_int_sig, g_int_sig) / int_sig / int_sig
            )
            - h_int_sig / (1 - self.w_bkg)
        )
        return ret, g, -h
