...
def build_tf_nll_grad(fcn, dtype=None, check=True, rtol=None, atol=None):
    """Build a compiled version of ``fcn.nll_grad``.

    The returned callable keeps the same interface as ``fcn.nll_grad``: it
    takes a list/array of physical parameter values and returns
    ``(nll, grad)`` (python float and numpy array). It can therefore be used
    as a drop-in replacement inside ``vm.trans_fcn_grad``.

    Two modes are supported:

    - if ``fcn`` exposes ``get_nll_grad_tf`` (e.g. ``FCN``), each batch is
      compiled separately and the batch loop stays eager, which is compatible
      with streaming (``lazy_call``) data;
    - otherwise a whole evaluation via ``get_nll_grad`` is wrapped in a single
      ``tf.function``.

    ``RuntimeError`` is raised when neither interface is available so the
    caller can fall back to the eager path.
    """
    import tensorflow as tf

    from tf_pwa.config import get_config

    if not hasattr(fcn, "gauss_constr"):
        raise RuntimeError(
            "tf.function nll_grad requires a FCN object with `gauss_constr`"
        )
    if hasattr(fcn, "get_nll_grad_tf"):
        batch_wise = True
    elif hasattr(fcn, "get_nll_grad"):
        batch_wise = False
    else:
        raise RuntimeError(
            "tf.function nll_grad requires a FCN object with "
            "`get_nll_grad_tf` or `get_nll_grad`"
        )
    ...
    def _stack_grad(g):
        if isinstance(g, (list, tuple)):
            return tf.stack(
                [tf.cast(tf.convert_to_tensor(gi), dtype) for gi in g]
            )
        return tf.cast(tf.convert_to_tensor(g), dtype)

    def _inner(y):
        vm.set_all(y)
        if batch_wise:
            nll, g = fcn.get_nll_grad_tf({})
        else:
            nll, g = fcn.get_nll_grad({})
        return tf.cast(nll, dtype), _stack_grad(g)

    if batch_wise:
        inner = _inner
    else:
        inner = tf.function(_inner)

    def fcn_grad(yvals):
        y = tf.convert_to_tensor(np.asarray(yvals), dtype=dtype)
        nll, g = inner(y)
        constr = fcn.gauss_constr.get_constrain_term()
        constr_grad = np.asarray(
            [float(i) for i in fcn.gauss_constr.get_constrain_grad()],
            dtype=np.float64,
        )
        return nll + constr, np.asarray(g, dtype=np.float64) + constr_grad

    if check:
        if rtol is None:
            rtol = 1e-8 if "64" in str(dtype) else 1e-4
        if atol is None:
            atol = rtol
        y0 = np.asarray(fcn.vm.get_all_val(), dtype=np.float64)
        nll_ref, g_ref = fcn.nll_grad(y0)
        nll_new, g_new = fcn_grad(y0)
        consistent = np.isclose(
            float(nll_ref), float(nll_new), rtol=rtol, atol=atol
        ) and np.allclose(
            np.asarray(g_ref, dtype=np.float64),
            np.asarray(g_new, dtype=np.float64),
            rtol=rtol,
            atol=atol,
        )
        if not consistent:
            raise RuntimeError(
                "tf.function nll_grad inconsistent with eager path: "
                "nll {} vs {}, grad {} vs {}".format(
                    float(nll_ref), float(nll_new), g_ref, g_new
                )
            )
