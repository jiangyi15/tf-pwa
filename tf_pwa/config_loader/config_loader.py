...
from tf_pwa.data import data_index, data_shape, data_split, load_data, save_data, data_merge
...
class ConfigLoader(object):
    ...
    def cal_bins_numbers(self, adapter, data, phsp, read_data, bg=None, bg_weight=None):
        ...
        phsp_cut = read_data(phsp) # np.array([data_index(phsp, idx) for idx in data_idx])
        ...
        weight_scale = self.config["data"].get("weight_scale", False)
        if bg is not None:
            bg_cut = read_data(bg) # np.array([data_index(bg, idx) for idx in data_idx])
    ...
    def cal_chi2(self, read_data=None, bins=[[2,2]]*3, mass=["R_BD", "R_CD"]):
        if read_data is None:
            ...
            read_data = lambda a: np.array([data_index(a, idx)**2 for idx in data_idx])
        data, bg, phsp, _ = self.get_all_data()
        bg_weight = self._get_bg_weight()[0]
        all_data = data_merge(*data)
        all_data_cut = read_data(all_data) # np.array([data_index(a, idx) for idx in data_idx])
