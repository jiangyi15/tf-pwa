...
def test_weight():
    ...
    assert np.allclose(hist1.get_count(), hist3.get_count())
    assert np.allclose(hist2.get_count(), hist4.get_count())
    assert np.allclose(hist1.scale_to(hist2), 1 / np.mean(weight))
