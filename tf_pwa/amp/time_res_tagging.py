...
def conv_exp_gaussian_with_resolution(
    t, sigma, gamma, t_min=0.0, res_p0=1.0, res_p1=0.0, time_bias=0.0
):
    """
...
def conv_exp_gaussian_complex_with_resolution(
    t, sigma, gamma, delta_m, t_min=0.0, res_p0=1.0, res_p1=0.0, time_bias=0.0
):
    """
...
class TimeDepCpConvResAmplitudeModel(TimeDepCpConvAmplitudeModel):
    ...
    def eval_P_Pbar_time(self, data):
        ...
        if (
            hasattr(self, "time_acceptance")
            and self.time_acceptance is not None
        ):
        ...
        if (
            hasattr(self, "angular_acceptance")
            and self.angular_acceptance is not None
        ):
            ...
            if angles is not None:
                ...
                ctK = tf.cos(theta_K)
                trigger = data.get("trigger", tf.zeros_like(t))
                eps_omega = self.angular_acceptance(
                    ctK, ctL, phi, trigger=trigger
                )
                ret1 = ret1 * eps_omega
                ret2 = ret2 * eps_omega
...
class FlavourTagLHCbPDF(BaseAmplitudeModel):
    ...
    def __init__(
        self, *args, tagger_configs=None, tag_eff=1.0, prod_asym=0.0, **kwargs
    ):
        """
        Args:
    ...
    def combine_taggers(self, tag_dec_list, eta_cal_list):
        ...
        denom_diff_p = tf.where(
            use_tagger1,
            wp1 * (1 - wp2) + (1 - wp1) * wp2,
            wp2 * (1 - wp1) + (1 - wp2) * wp1,
        )
        denom_diff_m = tf.where(
            use_tagger1,
            wm1 * (1 - wm2) + (1 - wm1) * wm2,
            wm2 * (1 - wm1) + (1 - wm2) * wm1,
        )
    ...
    def pdf(self, data):
        ...
        for i, config in enumerate(self.tagger_configs):
            ...
            eta_cal = self.calibrate_eta(
                eta, eta_mean, p0, p1, dp0, dp1, true_tag
            )
        ...
        prob = tf.where(
            ...
            0.5,  # Untagged
            tf.where(
                same_tag, 1 - comb_eta, comb_eta  # Correct tag  # Wrong tag
            ),
        )
        ...
        for i, config in enumerate(self.tagger_configs):
            ...
            if not hasattr(top, f"p0_{name}_{year}"):
                setattr(
                    ...
                    f"p0_{name}_{year}",
                    top.add_var(
                        f"p0_{name}_{year}", value=config.get("p0", 0.38)
                    ),
                )
                setattr(
                    top,
                    f"p1_{name}_{year}",
                    top.add_var(
                        f"p1_{name}_{year}", value=config.get("p1", 0.85)
                    ),
                )
                setattr(
                    top,
                    f"p0bar_{name}_{year}",
                    top.add_var(
                        f"p0bar_{name}_{year}",
                        value=config.get("p0bar", config.get("p0", 0.38)),
                    ),
                )
                setattr(
                    top,
                    f"p1bar_{name}_{year}",
                    top.add_var(
                        f"p1bar_{name}_{year}",
                        value=config.get("p1bar", config.get("p1", 0.85)),
                    ),
                )
    ...
    for tagger_name in ["TaggingOS", "TaggingSSK", "TaggingIFT"]:
        ...
        for year, params in year_params.items():
            config = {
                ...
                "year": year,
                "p0": params.get(
                    f"p0_{tagger_name.replace('Tagging', '')}", 0.38
                ),
                "p1": params.get(
                    f"p1_{tagger_name.replace('Tagging', '')}", 0.85
                ),
                "dp0": params.get(
                    f"dp0_{tagger_name.replace('Tagging', '')}", 0.0
                ),
                "dp1": params.get(
                    f"dp1_{tagger_name.replace('Tagging', '')}", 0.0
                ),
            }
...
def create_tagger_configs_from_json(
    json_path, taggers=["OS", "SSK"], years=[2016, 2017, 2018]
):
    """
