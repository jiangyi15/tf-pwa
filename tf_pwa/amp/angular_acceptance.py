...
class AngularAcceptance:
    ...
    def _compute_c_coefficients(self, omega):
        ...
        c1 = (1.0 / pi) * (
            1755.0 / 512.0 * omega_1
            + 135.0 / 128.0 * omega_2
            + 135.0 / 128.0 * omega_3
            - 2565.0 / 512.0 * omega_7
        )
        c2 = (1.0 / pi) * (
            135.0 / 128.0 * omega_1
            + 315.0 / 64.0 * omega_2
        ...
        c7 = (1.0 / pi) * (
            -2565.0 / 512.0 * omega_1
            - 405.0 / 128.0 * omega_2
            - 405.0 / 128.0 * omega_3
            + 6075.0 / 512.0 * omega_7
        )
        ...
        return np.array(
            [c1, c2, c3, c4, c5, c6, c7, c8, c9, c10], dtype=np.float64
        )
    ...
    def __call__(self, cos_theta_K, cos_theta_L, phi, trigger=None):
        ...
        if trigger is not None:
            trigger_np = (
                trigger.numpy()
                if hasattr(trigger, "numpy")
                else np.array(trigger)
            )
            c = np.where(
                trigger_np[..., None] == 1,
                self.c_biased,
            ...
            norm = np.where(
                trigger_np == 1, self.norm_biased, self.norm_unbiased
            )
        else:
