...
class BSplineTimeAcceptance:
    ...
    def __init__(
        self,
        knots=None,
        coefficients_unbiased=None,
        coefficients_biased=None,
        t_min=0.3,
        t_max=15.0,
    ):
        if knots is None:
    ...
    def _linear_extrapolate(self, t_np, coefficients, trigger_val=None):
        ...
        s_at_max = self._eval_spline(np.array([self.max_knot]), coefficients)[
            0
        ]
        s_at_minus = self._eval_spline(
            np.array([self.max_knot - self._h]), coefficients
        )[0]
